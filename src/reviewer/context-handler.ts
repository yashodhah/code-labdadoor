...
import type { AgentScope, ChangedFileMetadata, PatchFileRef, ReviewContext } from '../types/context';
...
export function writeSharedContext(ctx: ReviewContext): string {
  ...
  try {
    ...
    console.log('[context] Wrote context.json to', contextPath, '- file size:', JSON.stringify(content).length, 'bytes');
  } catch (err) {
...
export async function prepareContext(
  ...
  const patchRefs = files.map((file): PatchFileRef => ({
