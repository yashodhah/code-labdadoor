...
function parseXmlFinding(
  ...
  const resolvedCategory: Category = VALID_CATEGORIES.includes(category)
    ? category
    : fallbackCategory;
  const resolvedConfidence: Confidence = VALID_CONFIDENCES.includes(confidence)
    ? confidence
    : "medium";
...
export function parseXmlFindings(
  raw: string,
  agentName: string,
  fallbackCategory: Category,
): SpecialistFinding[] {
...
function buildOrchestratorSystemPrompt(
  ...
  const agentDescriptions = scopes
    .map((scope) => {
...
async function buildOrchestratorUserPrompt(scopes: AgentScope[]): Promise<string> {
  ...
  const agentSections = await Promise.all(
    scopes.map(async (scope) => {
      ...
      return [`## AGENT: ${scope.agentName}`, ...patchBlocks].join("\n");
    }),
  );
