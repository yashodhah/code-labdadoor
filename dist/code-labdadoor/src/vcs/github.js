...
exports.GitHubVcsProvider = void 0;
...
async function isGeneratedFile(octokit, owner, repo, filename, ref) {
    try {
...
function parseRepo(repoStr) {
    ...
    if (!owner || !repo)
        throw new Error(`Invalid repo format: "${repoStr}"`);
    return { owner, repo };
}
class GitHubVcsProvider {
    ...
    async postReview(decision, body) {
        const octokit = this.octokit();
        const { owner, repo, prNumber } = this.prCoords();
...
exports.GitHubVcsProvider = GitHubVcsProvider;
